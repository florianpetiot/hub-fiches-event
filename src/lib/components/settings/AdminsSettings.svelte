...
<div class="space-y-6">
  ...
  <section class="bg-dark-secondary shadow rounded-lg p-6 space-y-4">
    <h2 class="text-lg font-semibold text-text-main border-b border-dark-primary pb-2">Rôles</h2>
    <p class="text-sm text-text-muted">Les rôles "Club" et "Direction" sont fixes et ne peuvent pas être supprimés.</p>
    ...
    <div class="space-y-3">
      <div class="space-y-2">
        ...
        <div>
          {#if savingCreerRole || savingSupprimerRole}<p class="text-text-muted text-xs">Enregistrement en cours...</p>{/if}
          {#if showSupprimerRoleFeedback && hasError(actionData?.supprimerRole)}<p class="text-dark-red-accent text-xs">{actionData.supprimerRole.error}</p>{/if}
          {#if showSupprimerRoleFeedback && hasSuccess(actionData?.supprimerRole)}<p class="text-dark-green-accent text-xs">✓ Rôle supprimé</p>{/if}
        </div>
        ...
        <form method="POST" action="?/creerRole" use:enhance={() => {
          ...
          <div class="flex justify-between items-center">
            <div>
              {#if showCreerRoleFeedback && hasError(actionData?.creerRole)}<p class="text-dark-red-accent text-xs">{actionData.creerRole.error}</p>{/if}
              {#if showCreerRoleFeedback && hasSuccess(actionData?.creerRole)}<p class="text-dark-green-accent text-xs">✓ Rôle créé</p>{/if}
            </div>
          </div>
        </form>
      </div>
    ...
    {#if confirmDeleteRole}
      <ConfirmModal
        ...
        description="Cette action est irréversible. Les références liées au rôle seront supprimées. Pour confirmer, écrivez <strong class='text-text-main font-mono'>supprimer {nameRoleToDelete}</strong> ci-dessous."
  ...
  <section class="bg-dark-secondary shadow rounded-lg p-6 space-y-4">
        ...
        <div class="flex items-center justify-between gap-4 bg-dark-secondary border border-dark-primary rounded-lg px-4 py-3">
          <div class="flex flex-1 justify-between flex-wrap gap-2 items-center min-w-0">
            <div class="min-w-0 grow shrink basis-50">
              <p class="text-text-main text-sm font-medium truncate" title={admin.name}>{admin.name}</p>
              <p class="text-text-muted text-xs truncate" title={admin.email}>{admin.email}</p>
              <p class="text-text-muted text-xs truncate">Créé le {formatDate(admin.created_at)}</p>
            </div>
            <select
              value={admin.role_id}
    ...
    <div>
      {#if savingCreerAdmin || savingSupprimerAdmin || savingChangerRoleAdmin}<p class="text-text-muted text-xs">Enregistrement en cours...</p>{/if}
      {#if showChangerRoleAdminFeedback && hasError(actionData?.changerRoleAdmin)}<p class="text-dark-red-accent text-xs">{actionData.changerRoleAdmin.error}</p>{/if}
      {#if showChangerRoleAdminFeedback && hasSuccess(actionData?.changerRoleAdmin)}<p class="text-dark-green-accent text-xs">✓ Rôle mis à jour</p>{/if}
      {#if showSupprimerAdminFeedback && hasError(actionData?.supprimerAdmin)}<p class="text-dark-red-accent text-xs">{actionData.supprimerAdmin.error}</p>{/if}
      {#if showSupprimerAdminFeedback && hasSuccess(actionData?.supprimerAdmin)}<p class="text-dark-green-accent text-xs">✓ Compte supprimé</p>{/if}
    </div>

    {#if confirmChangeRoleAdmin}
      <ConfirmModal
        title="Changer le rôle ?"
        description="Le rôle de {confirmChangeRoleAdmin.profile_name} va devenir <b>{confirmChangeRoleAdmin.new_role_label}</b>. Si son ancien rôle n'est plus utilisé par aucun compte, ses étapes de workflow associées seront supprimées. Pour confirmer, écrivez <strong class='text-text-main font-mono'>modifier {confirmChangeRoleAdmin.profile_name}</strong> ci-dessous."
    ...
    {#if confirmDelete}
      <ConfirmModal
        ...
        description="Cette action est irréversible. L'ensemble des messages et signatures associées à {nameAdminToDelete} seront supprimés. Pour confirmer, écrivez <strong class='text-text-main font-mono'>supprimer {nameAdminToDelete}</strong> ci-dessous."
    ...
    <form method="POST" action="?/creerAdmin" use:enhance={() => {
      ...
      <h3 class="text-text-main text-sm font-medium">Inviter un administrateur</h3>
      <div class="grid grid-cols-2 gap-3">
        <div>
          <label for="new-admin-name" class="block text-xs text-text-muted uppercase mb-1">Nom</label>
        ...
        <div>
          <label for="new-admin-email" class="block text-xs text-text-muted uppercase mb-1">Email</label>
      ...
      <div class="flex justify-between items-center">
        <div>
          {#if showCreerAdminFeedback && hasError(actionData?.creerAdmin)}<p class="text-dark-red-accent text-xs">{actionData.creerAdmin.error}</p>{/if}
  ...
  <section class="bg-dark-secondary shadow rounded-lg p-6 space-y-4 mb-6">
    ...
    <div class="space-y-2">
      ...
      <div class="flex items-center gap-3 bg-dark-primary/50 rounded-lg px-4 py-3">
        <span class="w-6 h-6 rounded-full bg-dark-secondary text-text-muted flex items-center justify-center text-xs font-bold shrink-0">0</span>
        <p class="text-text-muted text-sm">Soumission par le club</p>
      </div>

      {#each data.workflow ?? [] as etape, i}
        <div class="flex items-center gap-3 bg-dark-primary rounded-lg px-4 py-3">
          <span class="w-6 h-6 rounded-full bg-accent-selection text-white flex items-center justify-center text-xs font-bold shrink-0">
            {etape.ordre}
          </span>
          <p class="text-text-main text-sm font-bold flex-1">{etape.roles?.label}</p>
      ...
      <div class="flex items-center gap-3 bg-dark-primary/50 rounded-lg px-4 py-3">
        <span class="w-6 h-6 rounded-full bg-dark-secondary text-text-muted flex items-center justify-center text-xs font-bold shrink-0">
          {(data.workflow?.length ?? 0) + 1}
        </span>
        <p class="text-text-muted text-sm">Validation finale — Direction</p>
      </div>
    </div>
    ...
    {:else}
      <p class="text-text-muted text-sm italic border-t border-dark-primary pt-4">
        Tous les rôles signataires sont déjà dans le workflow.
      </p>
    {/if}

    {#if savingWorkflow}<p class="text-text-muted text-xs">Enregistrement en cours...</p>{/if}
    {#if showWorkflowFeedback && hasError(actionData?.workflow)}<p class="text-dark-red-accent text-xs">{actionData.workflow.error}</p>{/if}
    {#if showWorkflowFeedback && hasSuccess(actionData?.workflow)}<p class="text-dark-green-accent text-xs">✓ Workflow mis à jour</p>{/if}
  </section>

  {#if confirmDeleteEtape}
    <ConfirmModal
      title="Supprimer l'étape ?"
      description="Cette action est irréversible. Les signatures en attente associées à cette étape seront supprimées. Les signatures déjà obtenues seront conservées. Pour confirmer, écrivez <strong class='text-text-main font-mono'>supprimer {nameEtapeToDelete}</strong> ci-dessous."
