...
<div class="space-y-2">
  ...
  {#if currentPath}
    <div class="flex items-center gap-3">
      ...
      <button type="button" onclick={() => fileInput?.click()}
        ...
        title="Remplacer"
        class="text-xs text-text-muted hover:text-text-main active:text-text-main flex items-center hover:cursor-pointer">
      ...
      <button type="button" onclick={handleDelete}
        disabled={deleting}
        title="Supprimer"
        class="text-xs text-dark-red-accent hover:text-dark-red-accent/80 active:dark-red-accent/80 flex items-center hover:cursor-pointer">
        <svg class="w-4 h-4 sm:hidden" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  {:else}
    <button type="button" onclick={() => fileInput?.click()}
      disabled={uploading}
      class="flex items-center gap-2 px-4 py-2 border border-dashed border-dark-primary rounded-lg text-sm text-text-muted hover:text-text-main active:text-text-main hover:border-text-main active:border-text-main transition-colors disabled:opacity-50 hover:cursor-pointer">
      {#if uploading}
